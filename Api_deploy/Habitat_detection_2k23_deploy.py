...
def image_to_grey_sort_resize(input_path, dest_path2):
    ...
    for i in os.listdir(input_path):
        ...
        if img is None:
            logger.error(f"Failed to read image at {input_path + i}")
            continue
...
def func3(image,bounds,scale):
    ...
    try:
        ...
        logger.error("Failed to create directory for input images: " + str(error))
        return Response(response='Error: Failed to create directory for input images', status=500)
    '''--------------------------------------------------------------------'''
    ...
    try:
        ...
        logger.error("Failed to create directory for masks: " + str(error))
        return Response(response='Error: Failed to create directory for masks', status=500)
    '''--------------------------------------------------------------------'''
...
def func2(image,scale):
    ...
    url = 'http://10.10.21.228:7005/satellite_model_2/'
    files = [('file', (filename, static_file.read(), content_type)), ('mask', mask)]
...
def func1(image,scale):
    ...
    cv2.imwrite(path1+"/"+"3_data.jpeg", image)
    ...
    url = 'http://10.10.21.228:7005/satellite_model_1/'
    files = [('file', (filename, static_file.read(), content_type)), ('mask', mask)]
...
fh = logging.FileHandler('/mnt/vol2/Dhruv_Raghav/general_unet_model/Api_deploy/Logs/error.log')
